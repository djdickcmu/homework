 ...
 * classes. It builds a binary tree with 4 nodes that are constant values. The parents
 * are binary operations. Once the tree is built it calls the ToString methods to display the 
...
public class TestAlgebra {
	...
	public static void main(String[] args) {
		...
		Node n;
		for (int i = 0; i < 5; i++) {
			n = randOperator(randOperator(randConstOrVariable(), randConstOrVariable()),
					randOperator(randConstOrVariable(), randConstOrVariable()));
			System.out.format("{X0, X1, X2} = {1, 2, 3} " + n.toString() + " = %.2f%n", n.eval(doubleArray1));
			System.out.format("{X0, X1, X2} = {4, 5, 6} " + n.toString() + " = %.2f%n", n.eval(doubleArray2));
			System.out.println();
		}
	...
	// operator node.
	// The binary operation is picked randomly.
